...
with st.form("tip_form"):
    total_bill = st.slider(
        "Total Bill ($)", min_value=0.0, max_value=500.00, value=20.0)
...
# Should show <class 'sklearn.pipeline.Pipeline'>
    ...
    try:
        ...
        predicted_tip = prediction[0] if isinstance(
            prediction, (list, np.ndarray)) else prediction
